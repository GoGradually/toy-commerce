...
public class CartService {
    ...
    public CartInfo getCartItems(Long memberId) {
        ...
        for (CartItem cartItem : cart.getItems()) {
            var activeProduct = productSnapshotPort.findActiveProduct(cartItem.getProductId());
            if (activeProduct.isEmpty()) {
                continue;
            }

            ProductSnapshot product = activeProduct.get();
            BigDecimal lineTotal = product.price().multiply(BigDecimal.valueOf(cartItem.getQuantity()));
    ...
    private void ensureProductCanBeAdded(Long productId) {
        ...
        productSnapshotPort.getActiveProduct(productId);
    }
