...
public class AdminProductController {
    ...
    @PatchMapping("/{productId}/force-sold-out")
    @Operation(summary = "상품 강제 품절", description = "미결제 주문을 취소하고 상품을 비활성/재고 0 상태로 전환합니다.")
    @ApiResponses(value = {
    ...
    public ApiResponse<ProductResponse> forceSoldOut(
            @Parameter(description = "상품 ID", example = "1")
            @PathVariable @Min(value = 1, message = "productId는 1 이상이어야 합니다.") Long productId
    ) {
        ProductDetailInfo updated = adminProductService.forceSoldOut(productId);
        return ApiResponse.success(ProductResponse.from(updated));
    }
