...
class ToyCommerceExceptionErrorCodeMapperTest {
    ...
    @Test
    void shouldMapDeleteConflict() {
        ErrorCode result = mapper.map(new ProductReferencedByOrderException(1L));

        assertThat(result).isEqualTo(ErrorCode.PRODUCT_DELETE_CONFLICT);
    }
    ...
    @Test
    void shouldMapOrderProductNotFound() {
        ErrorCode result = mapper.map(new OrderProductNotFoundException(1L));
