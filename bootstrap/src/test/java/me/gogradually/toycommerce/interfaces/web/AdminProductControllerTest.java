...
class AdminProductControllerTest {
    ...
    @Test
    void shouldForceSoldOut() throws Exception {
        ProductDetailInfo product = new ProductDetailInfo(
                1L,
                "레고 스타터 세트",
                new BigDecimal("15900"),
        ...
        when(adminProductService.forceSoldOut(1L)).thenReturn(product);

        mockMvc.perform(patch("/api/admin/products/1/force-sold-out"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    @Test
    void shouldReturnConflictWhenDeletingReferencedProduct() throws Exception {
        doThrow(new ProductReferencedByOrderException(1L))
